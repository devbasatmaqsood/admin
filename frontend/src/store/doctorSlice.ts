...
export const getPendingRequest = createAsyncThunk(
  ...
  async (payload: AllUsersPayload | undefined) => {
    return apiInstanceFetch.get(`admin/doctorRequest?type=1&start=${payload?.start}&limit=${payload?.limit}`);
  }
);
...
export const getParticularDoctorEarning = createAsyncThunk(
  ...
  async (payload: AllUsersPayload | undefined) => {
    return apiInstanceFetch.get(
      `admin/doctorWallet?doctorId=${payload?.doctorId}&type=${payload?.status}`
    );
  }
);
