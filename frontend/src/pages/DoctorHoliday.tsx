  ...
  const handleDelete = (id: any) => {
    ...
    data
      .then((logouts) => {
        ...
        if (yes) {
          dispatch(deleteDoctorHoliday(id));
        }
      })
      .catch((err) => console.log(err));
  };

  const doctorHolidayTabel = [
    {
      Header: "No",
    ...
    },
    {
      Header: "Reason",
      Cell: ({ row }: { row: doctorHoliday }) => (
        <span className="text-capitalize fw-bold">{row?.reason}</span>
      ),
    },
    {
      Header: "Delete",
      ...
      <div className="row">
        <div className="inputData col-2">
          ...
          >
            ...
            {doctorHoliday?.doctorDropDown?.map((item: any, index) => {
              return (
                <>
